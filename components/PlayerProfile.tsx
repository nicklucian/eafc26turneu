    ...
    return (
        <div className={`${sizeClasses} rounded-full shadow-2xl relative overflow-hidden ring-4 ring-black/20 bg-black`}>
            <svg viewBox="0 0 100 100" className="w-full h-full transform transition-transform duration-700 hover:rotate-[360deg]" style={{ transform: `rotate(${identity.rotation}deg)` }}>
                ...
                {/* Lighting/Volume Overlay */}
                <circle cx="50" cy="50" r="50" fill={`url(#grad-${username})`} />
                
                {/* Initials overlay for identification */}
...
export const PlayerProfile: React.FC<{ user: User }> = ({ user }) => {
    ...
    const { matches, tournaments, teams, users, assignmentCache } = useMemo(() => {
        const _matches = db.getAllMatches();
        const _tournaments = db.getTournaments();
        const _teams = db.getGlobalTeams();
        const _users = db.getUsers();
        
        // Cache assignments per tournament to avoid excessive DB calls in loops
        const _assignmentCache: Record<string, Assignment[]> = {};
        _tournaments.forEach(t => {
            _assignmentCache[t.id] = db.getAssignments(t.id);
        });
    ...
    }, []); // Empty dependency array means this calculates once on mount (or re-mount)
    ...
    const history = useMemo(() => {
        ...
        // Sort by most recent (assuming end of array is newest) and map
        return [...allUserMatches].reverse().map(m => {
            ...
            // Resolve teams using the cache
            const tAssignments = assignmentCache[m.tournamentId] || [];
            const myAssign = tAssignments.find(a => a.userId === user.id);
            const oppAssign = tAssignments.find(a => a.userId === oppId);
    ...
    }, [matches, tournaments, teams, users, user.id, assignmentCache]);
            ...
            <div className="ea-card p-8 md:p-10 rounded-[2.5rem] border-white/5 relative overflow-hidden shadow-2xl group">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
                    ...
                    <div className="text-center md:text-left flex-1 min-w-0">
                        <h2 className="text-5xl md:text-6xl font-black italic tracking-tighter uppercase truncate text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">
                            {user.username}
                        </h2>
